...
data_norm43_prop <- data_norm43 %>%
  subset_samples(Period !="P0")%>%
 # transform_sample_counts( function(x) x/sum(x)*100)%>%
  filter_taxa( function(x) sum(x) > 0, TRUE)

# lefse_data43 <- filter_taxa(data_norm43_prop, function(x) mean(x) > 1e-3, TRUE)
lefse_data43 <- data_norm43_prop
#move to directory where you downloaded the lefse zip, in my case it was in downloads:
setwd("/home/yo39qed/lefse/code")

levels(sample_data(lefse_data43)$Recharge)
...
lefse.tbl <- lefse2(lefse_data43, class="Recharge" ,subclass = "Period"
)
...
############################################################################################################################
LDA_H43 <- read.delim("~/time-series analysis/input/lefse/Galaxy88-[B)_LDA_Effect_Size_(LEfSe)_on_data_87]_H43_LDAover2.lefse_internal_res", header=F)

names(LDA_H43) <- c("Feature", "log_of_the_highest_class_average", "class", "LDA_effect_size", "p-value")
...
dim(filter_3) # Recharge:56, Discharge:29
...
#write.table(tet, "H43/lefse_result_for_bar_plot.csv", sep=";", dec=",", col.names=NA)
...
ggsave("H43/lefse_with_subclass_genus_H43.png", width = 18, height = 12)
