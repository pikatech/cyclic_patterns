...
period_option <- c("P14","P15","P16","P17", "P18", "P19")
...
data_norm43_select <- subset_samples(data_norm43, Period==selected_period)

metadata <- data.frame(Env=sample_data(data_norm43_select)$Sam_cam, SourceSink1=sample_data(data_norm43_select)$Recharge, SourceSink=sample_data(data_norm43_select)$Recharge) # T3
library(plyr)
metadata$SourceSink <- "Source"
metadata$SourceSink[metadata$SourceSink1=="Recharge"] <- "Sink"
row.names(metadata) <- sample_names(data_norm43_select) # T3
metadata <- metadata[, c("Env", "SourceSink")]
otus <- vegan_otu(data_norm43_select)# T3
dim(otus) 
...
setwd("/home/yo39qed/time-series analysis/FEAST")
write.table(prop_merged, paste0("H43/feast_recharge_effect_", selected_period, ".csv"), sep=";", dec=",", col.names = NA)
...
setwd("/home/yo39qed/time-series analysis/FEAST")
t1 <- read.csv("H43/feast_recharge_effect_P14.csv", sep=";", dec = ",",  header=TRUE)
t2 <- read.csv("H43/feast_recharge_effect_P15.csv", sep=";", dec = ",",  header=TRUE)
t3 <- read.csv("H43/feast_recharge_effect_P16.csv", sep=";", dec = ",",  header=TRUE)
t4 <- read.csv("H43/feast_recharge_effect_P17.csv", sep=";", dec = ",",  header=TRUE)
t5 <- read.csv("H43/feast_recharge_effect_P18.csv", sep=";", dec = ",",  header=TRUE)
t6 <- read.csv("H43/feast_recharge_effect_P19.csv", sep=";", dec = ",",  header=TRUE)


t <- do.call("rbind", list(t1, t2, t3, t4, t5, t6))
...
res
write.table(res, "H43/feast_unknown_source_recharge_H43.csv", sep=";", dec=",", col.names = NA)
...
p_recharge_feast_H43 <- ggboxplot(prop_merged_unknown, x="Period", y = "Proportions_est", fill = "Period",
  ...
  stat_compare_means(label.x = 1.5, label.y = 5, size=6.5) +
  ...
  geom_hline(yintercept=mean(prop_merged_unknown$Proportions_est, na.rm=TRUE), linetype="dashed", 
             color = "black", size=1) +
  ggtitle("Unknown source contributing to H43 recharge communities")+
...
ggsave('H43/feast_Recharge_H43.pdf', width = 8, height = 4)
ggsave('H43/feast_Recharge_H43.jpg', width = 8, height = 4)


################################### Combine all wells ###################################
prop_merged_unknow_H43 <- prop_merged_unknown
prop_merged_unknow_H43$Well <- "H43"
