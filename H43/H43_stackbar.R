...
my_data_prop <- transform_sample_counts(data_norm43, function(x) 100*x/sum(x))
#my_data_prop <- transform_sample_counts(my_data43, function(x) 100*x/sum(x))
...
# save(final, file ="H43/final_H43.phyloseq")
...
write.table(my_data_rank_melt, "H43/Class_over1percent_per_well.csv", sep=";", dec=",", col.names=NA)
...
my_data_rank_melt_select$Class<- factor(my_data_rank_melt_select$Class, levels = 
                                          c("Others", "Bacteria_unclassified",
                                            "Parcubacteria", "ABY1",  "Saccharimonadia" , "Gracilibacteria", "Berkelbacteria",
...
a_H43 <- ggplot(my_data_rank_melt_select,
...
a_H43
ggsave("H43/stacked_area_class_H43_mothur-unsorted_norm.pdf", width = 9.45, height = 7)
...
tb_OTU <- my_data_prop %>%
  ...
  ungroup
write.table(tb_OTU, "H43/mean_SD_relativE_abundance_OTU.csv", sep=";", dec=",", col.names=NA)
...
tb_genus <- my_data_prop %>%
  ...
  summarize(Mean = mean(Abundance), SD = sd(Abundance)) %>%
  ungroup
write.table(tb_genus, "H43/mean_SD_relativE_abundance_genus.csv", sep=";", dec=",", col.names=NA)
...
tb_family <- my_data_prop %>%
  ...
  summarize(Mean = mean(Abundance), SD = sd(Abundance)) %>%
  ungroup
write.table(tb_family, "H43/mean_SD_relativE_abundance_family.csv", sep=";", dec=",", col.names=NA)
...
tb_order <- my_data_prop %>%
  ...
  summarize(Mean = mean(Abundance), SD = sd(Abundance)) %>%
  ungroup
write.table(tb_order, "H43/mean_SD_relativE_abundance_order.csv", sep=";", dec=",", col.names=NA)
...
tb_class <- my_data_prop %>%
  ...
  summarize(Mean = mean(Abundance), SD = sd(Abundance)) %>%
  ungroup
write.table(tb_class, "H43/mean_SD_relativE_abundance_class.csv", sep=";", dec=",", col.names=NA)
...
tb_phylum <- my_data_prop %>%
  ...
  summarize(Mean = mean(Abundance), SD = sd(Abundance)) %>%
  ungroup
write.table(tb_phylum, "H43/mean_SD_relative_abundance_phylum.csv", sep=";", dec=",", col.names=NA)
