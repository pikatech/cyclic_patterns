...
lefse.tbl <- lefse2(lefse_data41, class="Recharge" ,subclass = "Period"
)
# got error message: "Error in file(file, "rt") : cannot open the connection"
# with subclass (Period): P0 is removed
# without subclass (Period): p0 is kept

# However, we got the lefse.txt file and can use it in the galaxy server at: https://huttenhower.sph.harvard.edu/galaxy/
...
# 2, 5, 4, 5 # LDAover3
# 2, 4, 4, 4 # LDAover2, not working well, too many features
...
#write.table(tet, "H41/lefse_result_for_bar_plot.csv", sep=";", dec=",", col.names=NA)
...
tet_family %>%
  mutate(LDA_effect_size=ifelse(class=="Discharge",-LDA_effect_size,LDA_effect_size)) -> tet3 # reversed LDA values in 2 directions

tet3 <- subset(tet3, Family != "<NA>")
# Change the names a bit, because some genus share the same name but belong to different families
attach(tet3)
#detach(tet2)
lb_f <- str_split_fixed(tet3[order(LDA_effect_size),]$Feature , "\\.", 6)[ ,5] # [, 6] is genus level, [, 5]: family, etc 
...
tet3 %>%
  ...
  # ggtitle("LEfSe")+
  # theme(plot.margin=unit(c(8,1,1,1), "lines"))+ # top, right, bottom,left
  coord_flip() + annotate("label", y = -2.5, x = 40, size=10,
                          label ="Recharge-depressed", fill="grey", color="white")+
  annotate("label", y = 2, x = 25, size=10,
           label ="Recharge-favored", fill="orange", color="white") -> p3
