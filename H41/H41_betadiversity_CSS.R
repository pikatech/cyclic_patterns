...
# data_norm41_select <- subset_samples(data_norm41, Period !="Period0")
data_norm41_select <- data_norm41
data_norm41_select <- filter_taxa(data_norm41_select , function(x) sum(x) > 0, TRUE)

#####
set.seed(123)
data_norm41_bray <- phyloseq::distance(data_norm41_select, method = "bray") 
# make a data frame from the sample_data
sampledf <- data.frame(sample_data(data_norm41_select))

# Adonis test (Number of permutations: 999)
set.seed(123)
ht_RechargePeriod <- adonis(data_norm41_bray ~ Recharge + Period + Recharge*Period, data = sampledf) #  ***
...
### Pairwise comparison ##
set.seed(123)
adonis_pair_Period <- pairwise.adonis(vegan_otu(data_norm41_select), sampledf$Period)
adonis_pair_Period
write.table(data.frame(adonis_pair_Period), "H41/bray/permanova_Period_pairwise_H41.csv", sep=";", dec=",", col.names=NA)

####
set.seed(123)
adonis_pair_PeriodRecharge <- pairwise.adonis(vegan_otu(data_norm41_select), sampledf$PeriodRecharge)
adonis_pair_PeriodRecharge
write.table(data.frame(adonis_pair_PeriodRecharge), "H41/bray/permanova_PeriodRecharge_pairwise_H41.csv", sep=";", dec=",", col.names=NA)

# Homogeneity of dispersion test (Number of permutations: 999)
library(vegan)
dis_Period<- betadisper(data_norm41_bray, sampledf$Period)
set.seed(123)
permutest(dis_Period) # 0.003**

# Homogeneity of dispersion test (Number of permutations: 999)
library(vegan)
dis_PeriodRecharge<- betadisper(data_norm41_bray, sampledf$PeriodRecharge)
set.seed(123)
...
set.seed(123)
ordu = ordinate(data_norm41, "PCoA", "bray")
scree <- plot_scree(ordu, "Scree plot for bacterial community, 0.2 filter, bray")
scree$data
tbd <- cbind(ordu$vectors[, 1:2], sample_data(data_norm41))
...
tp_final <- ggscatter(ag, x = "Axis.1.mean", y = "Axis.2.mean",
  ...
  ggtitle("H41: PCoA_Period_recharge")

tp_final #5*6

ggsave("H41/bray/pcoa_H41_PeriodRecharge_mean_linked.pdf", width = 7, height = 5)
