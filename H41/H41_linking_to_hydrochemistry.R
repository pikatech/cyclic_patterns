...
"H41"=  "#A2D9F7", "H42"= "lightgoldenrod1","H41" = "lightgoldenrod4",# well H4
...
dat_all <- data.frame(sample_data(data_norm41))
...
write.table(p.mat, "H41/env_correlation_pvalue_H41.csv", sep=";", dec=",", col.names = NA)
write.table(corr_mat, "H41/env_correlation_pearson_H41.csv", sep=";", dec=",", col.names = NA)
...
dev.copy(pdf,"H41/env_pearson_H41.pdf", width = 6, height = 6)
dev.off()
...
# Remove water temp and water level (because large data missing for H42 and H41)
...
# write.table(u3, "H41/env_cca_H41.csv", sep=";", dec=",", col.names=NA)

df1 <- na.omit(u3) # 74 reduced to 46 samples (with water temp), 53 (without water temp)

my_data_prop_sqrt_not_na <- data_norm41
...
my_data_prop_sqrt_not_na1 <- data_norm41
...
p_H41 <- cap_plot_env + 
  ...
  ggtitle("H41: Linking to environmental variables") +
 # xlim(-1.5, NA)+
...
ggsave("H41/cap_linking_env_H41.pdf", width = 9, height = 7)
