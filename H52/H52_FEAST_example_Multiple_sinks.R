...
data_norm52_select <- subset_samples(data_norm52, Period==selected_period)

metadata <- data.frame(Env=sample_data(data_norm52_select)$Sam_cam, SourceSink1=sample_data(data_norm52_select)$Recharge, SourceSink=sample_data(data_norm52_select)$Recharge) # T3
library(plyr)
metadata$SourceSink <- "Source"
metadata$SourceSink[metadata$SourceSink1=="Recharge"] <- "Sink"
row.names(metadata) <- sample_names(data_norm52_select) 
metadata <- metadata[, c("Env", "SourceSink")]
otus <- vegan_otu(data_norm52_select)
dim(otus) 
...
setwd("/home/yo39qed/time-series analysis/FEAST")
write.table(prop_merged, paste0("H52/feast_recharge_effect_", selected_period, ".csv"), sep=";", dec=",", col.names = NA)
...
setwd("/home/yo39qed/time-series analysis/FEAST")
t1 <- read.csv("H52/feast_recharge_effect_P15.csv", sep=";", dec = ",",  header=TRUE)
t2 <- read.csv("H52/feast_recharge_effect_P16.csv", sep=";", dec = ",",  header=TRUE)
t3 <- read.csv("H52/feast_recharge_effect_P17.csv", sep=";", dec = ",",  header=TRUE)
t4 <- read.csv("H52/feast_recharge_effect_P18.csv", sep=";", dec = ",",  header=TRUE)
t5 <- read.csv("H52/feast_recharge_effect_P19.csv", sep=";", dec = ",",  header=TRUE)
...
write.table(res, "H52/feast_unknown_source_recharge_H52.csv", sep=";", dec=",", col.names = NA)
...
p_recharge_feast_H52 <- ggboxplot(prop_merged_unknown, x="Period", y = "Proportions_est", fill = "Period",
 ...
 stat_compare_means(label.x = 1.5, label.y = 3, size=6.5) +
  ...
  geom_hline(yintercept=mean(prop_merged_unknown$Proportions_est, na.rm=TRUE), linetype="dashed", 
             color = "black", size=1) +
  ggtitle("Unknown source contributing to H52 recharge communities")+
...
p_recharge_feast_H52

setwd("/home/yo39qed/time-series analysis/output")
ggsave('H52/feast_Recharge_H52.pdf', width = 8, height = 4)
ggsave('H52/feast_Recharge_H52.jpg', width = 8, height = 4)


################################### Combine all wells ###################################
prop_merged_unknow_H52 <- prop_merged_unknown
prop_merged_unknow_H52$Well <- "H52"

######################################################################################
prop_merged_unknown <- do.call("rbind", list(prop_merged_unknow_H41, prop_merged_unknow_H43, prop_merged_unknow_H52))
prop_merged_unknown$Period
prop_merged_unknown$Period <- factor(prop_merged_unknown$Period, levels = c("P14", "P15", "P16", "P17", "P18", "P19"))

p_recharge_feast_H43 <- ggboxplot(prop_merged_unknown, x="Period", y = "Proportions_est", fill = "Period",
                                  palette = set_colors_Period,
                                  add = "mean" 
                                  #shape = "Aquifer",
  ...
  theme_bw()+
  ylab("Unknown source contribution (%)")+
  stat_compare_means(label.x = 1, label.y = 15, size=6.5) +
  facet_grid(Well~., scales='free')+
...
p_recharge_feast_H43

setwd("/home/yo39qed/time-series analysis/output")
ggsave('feast_Recharge_3wells.pdf', width = 10, height = 8)
ggsave('feast_Recharge_3wells.jpg', width = 10, height = 8)
