...
"H52"=  "#A2D9F7", "H42"= "lightgoldenrod1","H52" = "lightgoldenrod4",# well H4
...
dat_all <- data.frame(sample_data(data_norm52))
...
write.table(p.mat, "H52/env_correlation_pvalue_H52.csv", sep=";", dec=",", col.names = NA)
write.table(corr_mat, "H52/env_correlation_pearson_H52.csv", sep=";", dec=",", col.names = NA)
...
dev.copy(pdf,"H52/env_pearson_H52.pdf", width = 6, height = 6)
dev.off()
...
# Remove water temp and water level (because large data missing for H42 and H52)
...
# write.table(u3, "H52/env_cca_H52.csv", sep=";", dec=",", col.names=NA)

df1 <- na.omit(u3) # 74 reduced to 46 samples (with water temp), 53 (without water temp)

my_data_prop_sqrt_not_na <- data_norm52
...
my_data_prop_sqrt_not_na1 <- data_norm52
...
p_H52 <- cap_plot_env + 
  ...
  ggtitle("H52: Linking to environmental variables") +
 # xlim(-1.5, NA)+
...
ggsave("H52/cap_linking_env_H52.pdf", width = 9, height = 7)
